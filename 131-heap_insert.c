...
{
	heap_t *wzqtree, *wzqnew, *wzqflip;
	int wzqsize, wzqleaves, wzqsub, bit, wzqlevel, tmp;
	...
	for (bit = 1 << (wzqlevel - 1); bit != 1; bit >>= 1)
		wzqtree = wzqleaves & bit ? wzqtree->right : wzqtree->left;
	/*
	...
	wzqflip = wzqnew;
	for (; wzqflip->parent && (wzqflip->n > wzqflip->parent->n); wzqflip = wzqflip->parent)
	{
		tmp = wzqflip->n;
		wzqflip->n = wzqflip->parent->n;
		wzqflip->parent->n = tmp;
